...
from train_eval import train_full, eval_test, overlay_examples, save_context_adjacency_plots
...
def main():
    ...
    with h5py.File(cfg.mat_path, "r") as f:
        data_raw = np.array(f["neuralDataAllcontexts_reordered"])  # (4, trials, 80, 400)

    # transpose to (context, trial, channel, time) if needed
    data = np.transpose(data_raw, (0, 1, 2, 3)).astype(np.float32)
    num_contexts, num_trials, num_ch, T = data.shape
    assert num_contexts == 4 and num_ch == cfg.num_channels_total, "Data shape mismatch"

    # flatten to (all_trials, 80, 400)
    data = data.reshape(num_contexts * num_trials, num_ch, T)
    labels = np.repeat(np.arange(num_contexts), num_trials).astype(np.int64)

    # ----------------------------------------------------------
    # 2) Split into train / val / test  (context-balanced)
    # ----------------------------------------------------------
    ...
    save_context_adjacency_plots(model, cfg, region_labels=list(cfg.region_to_channels.keys()))
